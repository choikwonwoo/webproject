...
function btnClick(mode){
	var theForm = document.frmSend;
	if(mode == 'y'){
		theForm.action = "make_couple";
	} else if(mode == 'n'){
		theForm.action = "refuse_couple"
	}
	theForm.submit();
}
</script>
...
%>
<form name="frmSend" action="make_couple" method="post">
<input type="hidden" name="jdate" value="<%=cm.getCm_jdate() %>"/>
<input type="hidden" name="senderEmail" value="<%=cm.getCm_mail_s() %>"/>
<input type="hidden" name="receiverEmail" value="<%=loginInfo.getMi_mail()%>"/>
<%=cm.getCm_jdate() %><%=cm.getCm_mail_s() %><%=loginInfo.getMi_mail()%>
